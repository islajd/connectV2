...
const updateCashDrawerShift = async (cashDrawerShift) => {
    try {
        const key = cashDrawerShift.key()
        const rest = cashDrawerShift.cashDrawerShiftData
        ...
        let errorMessage = 'Could not update CashDrawerShift'
        // If it's a condition check violation, we'll try to indicate which condition failed.
        if (err.code === 'ConditionalCheckFailedException') {
            errorMessage = 'Given CashDrawerShift not found.'
        }
        return {
            error: errorMessage
        }
